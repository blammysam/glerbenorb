...
def split_sprite_sheet(path,sprite_size):
    ...
    sprites = []
    for row in range(sprites_in_column):
        for col in range(sprites_in_row):
            sprite = pygame.Surface(sprite_size,pygame.SRCALPHA)
            sprite.blit(sheet,(0,0),pygame.Rect(col*sprite_size[0],row*sprite_size[1],sprite_size[0],sprite_size[1]))
